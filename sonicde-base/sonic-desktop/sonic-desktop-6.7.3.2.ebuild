...
# LICENSE is based on Debian's plasma-desktop 4:6.5.4-2 d/copyright file.
LICENSE="GPL-2+ || ( GPL-2 LicenseRef-KDE-Accepted-GPL ) LGPL-2 LGPL-2+ LGPL-2.1+ || ( LGPL-2.1 LicenseRef-KDE-Accepted-LGPL ) BSD BSD-2 CC0-1.0 MIT"
SLOT="6"
if [[ "${PV}" != *9999 ]]; then
...
IUSE_INPUT_DEVICES="
	...
	input_devices_wacom
"
IUSE="${IUSE_INPUT_DEVICES} ibus scim screencast sdl +semantic-desktop webengine"
...
src_prepare() {
	...
	if ! use scim; then
		grep -q "^pkg_check_modules.*SCIM" CMakeLists.txt \
			|| die "Can't find pkg-config SCIM line."
		sed -e "s/^pkg_check_modules.*SCIM/#&/" -i CMakeLists.txt || die
	fi

	if ! use input_devices_evdev; then
		grep -q "^pkg_check_modules(XORGEVDEV" CMakeLists.txt \
			|| die "Can't find pkg-config evdev line."
		sed -e "s/^pkg_check_modules(XORGEVDEV/#&/" -i CMakeLists.txt || die
	fi

	if ! use input_devices_libinput; then
		grep -q "^pkg_check_modules(XORGLIBINPUT" CMakeLists.txt \
			|| die "Can't find pkg-config libinput line."
		sed -e "s/^pkg_check_modules(XORGLIBINPUT/#&/" -i CMakeLists.txt || die
	fi

	if ! use input_devices_synaptics; then
		grep -q "^pkg_check_modules(XORGSYNAPTICS" CMakeLists.txt \
			|| die "Can't find pkg-config synaptics line."
		sed -e "s/^pkg_check_modules(XORGSYNAPTICS/#&/" -i CMakeLists.txt || die
	fi
}

src_configure() {
	local touchpad
	if use input_devices_libinput || use input_devices_synaptics; then
		touchpad=ON
	else
		touchpad=OFF
	fi
	...
	local mycmakeargs=(
		...
		-DBUILD_KCM_TOUCHPAD_X11="${touchpad}"
